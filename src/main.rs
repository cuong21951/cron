...
fn cmd_autostart(remove: bool) -> Result<(), String> {
    ...
    let batch = PathBuf::from(&appdata).join(r"cron\autostart.cmd");

    if remove {
        let mut removed = false;
        for p in [&launcher, &batch] {
            if p.exists() {
    ...
    if let Some(dir) = batch.parent() {
        std::fs::create_dir_all(dir).ok();
    }

    // Batch wrapper does the actual launch + log redirect. Keeping the redirect
    // inside a `.cmd` avoids the cmd.exe quote-stripping trap that mangles
    // quoted paths when special chars like `>>` appear on the command line.
    let batch_content = format!(
        "@echo off\r\n\"{}\" run >> \"{}\" 2>&1\r\n",
        exe.display(),
        log.display()
    );
    std::fs::write(&batch, batch_content)
        .map_err(|e| format!("failed to write {}: {e}", batch.display()))?;

    // Hidden launcher (window style 0). `"""<path>"""` in VBScript yields the
    // runtime string `"<path>"`, i.e. the batch path safely quoted.
    let vbs_content = format!(
        "' Auto-generated by `cron autostart` - starts the scheduler hidden at logon.\r\n\
         CreateObject(\"WScript.Shell\").Run \"\"\"{}\"\"\", 0, False\r\n",
        batch.display()
    );
    std::fs::write(&launcher, vbs_content)
